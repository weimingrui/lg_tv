...
function addIEReflex(image) {
    var object, vml, display, flt, classes, newClasses, head, fill, flex, foot;
...
function addReflex(image) {
    var object, canvas, context, classes, newClasses, resource, tmp;
        ...
        if (tilt == "l" || tilt == "r") {
            if (resource.getContext) {
                ...
                if (canvas.getContext) {
                    ...
                    context.clearRect(0, 0, canvas.width, canvas.height);
                    if (tilt == "r") {
                        clipPolyRight(context, canvas.width / 20, 0, canvas.width * .95, canvas.height, iter + (rest > 0 ? 1 :0), dist, higt);
                    } else {
                        clipPolyLeft(context, canvas.width / 20, 0, canvas.width * .95, canvas.height, iter + (rest > 0 ? 1 :1), dist, higt);
                    }
                    context.clip();
                    context.drawImage(resource, parseInt(canvas.width / 20), 0, parseInt(canvas.width * .9), canvas.height);
                    context.save();
                    if (iborder > 0) {
                        context.lineWidth = iborder;
                        if (tilt == "r") {
                            strokePolyRight(context, canvas.width / 20, 0, canvas.width * .95, canvas.height, iter + (rest > 0 ? 1 :0), dist, higt, bord);
                            context.stroke();
                        } else {
                            strokePolyLeft(context, canvas.width / 20, 0, canvas.width * .95, canvas.height, iter + (rest > 0 ? 1 :0), dist, higt, bord);
                            context.stroke();
                        }
                    }
                    if (isWK) {
                        ...
                        context.fillStyle = stl;
                        clipReflex(context, canvas.width / 20, 0, canvas.width * .95, canvas.height, iter + (rest > 0 ? 1 :0), dist, higt, tilt);
                        context.fill();
                        globalCompositeOperation = "source-in";
                        clearReflex(context, canvas.width / 20, 0, canvas.width * .95, canvas.height, iter + (rest > 0 ? 1 :0), dist, higt, tilt);
                        context.clip();
                        context.clearRect(0, 0, canvas.width, canvas.height);
                        context.clearRect(0, 0, canvas.width, canvas.height);
                        context.clearRect(0, 0, canvas.width, canvas.height);
...
function invertedImage(obj) {
    if (isIE) addIEReflex(obj); else addReflex(obj);
}